...
class Smalls(m.Scene):
    def construct(self):
        ...
        text1 = m.MathTex("r = ",
                r"p_1^{e_1}\cdot p_2^{e_2}\cdot p_3^{e_3}\cdots p_k^{e_k}",
                r"0\leq e_i",
                r"k \textrm{ is the number of small primes}")
        # terms: text1[1][0:4] + text1[1][5:9] + text1[1][10:14] + text1[1][17:21]
        text2 = m.MathTex("r = ",
                r"x^2\cdot",
                r"p_1^{e_1}\cdot p_2^{e_2}\cdot p_3^{e_3}\cdots p_k^{e_k}",
                r"0\leq e_i",
                r"\leq 1 \quad x^2\leq n")
        one = m.MathTex(r"1\cdot{}")
        text1[0:2].next_to(title, m.DOWN, buff=1)
        text1[2].next_to(text1[0:2], m.DOWN)
        text1[3].next_to(text1[2], m.DOWN)
        text2[:3].move_to(text1[0:2])
        text2[3:].next_to(text2[:3], m.DOWN)
        one[0].move_to(m.Group(text2[1][0], text2[1][2]))

        self.play(m.Write(text1[0]))
        self.wait(5)
        for i, j in zip([0, 4, 9, 14], [4, 9, 14, 21]):
            self.play(m.Write(text1[1][i:j]))
            self.wait(1.8)
        self.wait(2)
        self.play(m.Write(text1[2]))
        self.play(m.Write(text1[3]), run_time=1)
        self.wait(7)
        ...
        self.play(m.AnimationGroup(text1[1][17:21].copy().animate.move_to(text2[1][0]).fade(1),
                                   ...
                                   lag_ratio=0.6))
        self.wait(5)
        self.play(m.Indicate(one[0][0], scale_factor=1.5, color=m.WHITE, rate_func=m.rate_functions.there_and_back_with_pause))
        self.wait(4)
        self.play(m.Circumscribe(one[0][0], m.Rectangle))
        self.play(text1[2].animate.move_to(text2[3]), m.FadeOut(text1[3]))
        self.wait(2)
        self.play(m.FadeIn(text2[4][:2]))
        self.wait(4.5)
        self.play(m.FadeIn(text2[4][2:]))
        ...
        self.play(m.Write(text3[0]))
        self.wait(2)
        self.play(m.Indicate(text2[4][2:]))
        self.wait(4)
        self.play(m.Write(text3[1:]))
        self.wait(2)
        self.play(m.Write(text4[0]))
        self.wait(2)
        self.play(m.Write(text4[1:]))
        self.wait(2)
        self.play(m.Write(text5[0]))
        self.wait(10)
        self.play(m.Write(text5[1:]))
        ...
        self.play(smalls.animate.to_corner(m.DOWN + m.LEFT),
                m.FadeOut(m.Group(title, text1[:3], text3, text4, one, text2[4])))
        self.wait()
